...
class JsPageMiddleware(object):
    def process_request(self, request, spider):
        ...
        import time, re
        if re.match("/?(.*toutiao.com/ch/.*)", request.url):
            for i in range(1):
                time.sleep(2)
                spider.brower.execute_script(
                    # 逐步下拉
                    "window.scrollBy(0, (document.body.scrollHeight)/2); var lenOfPage=(document.body.scrollHeight)/2; return lenOfPage;"
                )
        time.sleep(2)
